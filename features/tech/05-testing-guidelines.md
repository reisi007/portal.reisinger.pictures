...
- **On-the-fly Creation:** E2E tests MUST create their own isolated users on-the-fly via the API in `test.beforeAll()`. Use the `E2EUserHelper.createIsolatedUser()` utility to generate a fresh user with the required role (`admin`, `photographer`, `client`) for each test suite to prevent database state pollution and parallel execution conflicts.
