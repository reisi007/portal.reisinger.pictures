    ...
    return (
        <div className="flex h-screen bg-base-100">
            ...
            <main className="flex-1 overflow-y-auto flex flex-col">
                <header className="p-6 bg-base-100 border-b border-base-300 sticky top-0 z-10">
                        ...
                        {searchQuery.length >= 2 && searchResults && (
                            <div className="absolute top-14 left-0 w-full bg-base-100 shadow-2xl rounded-box border border-base-300 z-50">
                                <ul className="menu p-2">
                                    {searchResults.galleries.map(g => <li key={g.id}><a onClick={() => navigate("/g/" + g.slug)}>📁 {g.name}</a></li>)}
                                    {searchResults.photos.map(p => <li key={p.id}><a onClick={() => navigate("/p/" + p.id)}>{p.filename}</a></li>)}
                                </ul>
                            </div>
                        )}
                    </div>
