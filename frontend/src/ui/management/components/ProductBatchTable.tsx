        ...
        <div className="bg-base-100 border border-base-300 rounded-box p-4 md:p-6 shadow-sm mb-6 last:mb-0">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-2 mb-4">
                <div className="relative flex-1 w-full">
                    <span className="iconify mdi--magnify absolute left-3 top-1/2 -translate-y-1/2 text-base-content/50"></span>
                    <input 
                        type="text" 
                    ...
                    <div className="shrink-0 w-full sm:w-auto">
                        {isBatchMode ? (
                            <div className="flex gap-2 w-full">
                                <button className="btn btn-sm btn-ghost flex-1 sm:flex-none" onClick={() => setIsBatchMode(false)} disabled={isSaving}>Abbrechen</button>
                                <button className="btn btn-sm btn-primary flex-1 sm:flex-none" onClick={handleSave} disabled={isSaving}>
                ...
                <table className="table table-zebra w-full">
                    ...
                    <tbody>
                            ...
                            <tr key={p.id}>
                                <td>
                                    ...
                                    <div className="mt-1.5">
                                        ...
                                        {p.type === 'discount_fixed' && <span className="badge badge-warning badge-xs">Rabatt (€)</span>}
                                        {p.type === 'discount_percent' && <span className="badge badge-warning badge-xs">Rabatt (%)</span>}
                                    </div>
                                </td>
                                <td>
                                    {isBatchMode ? (
                                        ...
                                        <div className="text-xs opacity-70 leading-relaxed">{p.description || '-'}</div>
                                    )}
                                </td>
                                <td className="text-right align-middle">
                                    {isBatchMode ? (
                                        <label className="input input-sm input-bordered flex items-center gap-2 w-28 ml-auto">
                                            <input 
                                                type="number" 
                                                step="any"
                                                min="0"
                                            ...
                                            />
                                            <span className="opacity-60 font-bold shrink-0">{p.type === 'discount_percent' ? '%' : '€'}</span>
                                        </label>
                                    ) : (
                                ...
                                {!isBatchMode && (
                                    <td className="text-right align-middle">
                                        <div className="flex justify-end gap-1">
                                            <button className="btn btn-ghost btn-xs btn-square" title="Bearbeiten" onClick={() => onEdit(p)}>
                                                <span className="iconify mdi--pencil text-base"></span>
                                            </button>
                                            <button className="btn btn-ghost btn-xs btn-square text-error" onClick={() => onDelete(p.id)} title="Löschen">
                        ...
                        {filteredProducts.length === 0 && (
                            <tr><td colSpan={isBatchMode ? 3 : 4} className="text-center py-8 opacity-50">Keine Einträge gefunden.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            ...
            <div className="md:hidden flex flex-col gap-4">
                {filteredProducts.map(p => (
                    <div key={p.id} className="bg-base-200/50 border border-base-300 rounded-box p-4 relative flex flex-col gap-4 shadow-sm">
                        {!isBatchMode && (
                            <div className="absolute top-3 right-3 flex gap-1">
                                <button className="btn btn-ghost btn-sm btn-square h-8 w-8 min-h-0" onClick={() => onEdit(p)}>
                                    <span className="iconify mdi--pencil text-lg"></span>
                                </button>
                                <button className="btn btn-ghost btn-sm btn-square h-8 w-8 min-h-0 text-error" onClick={() => onDelete(p.id)}>
                        ...
                        <div className="flex flex-col gap-1">
                            <span className="text-xs font-bold opacity-70">Beschreibung</span>
                            {isBatchMode ? (
                                <input 
                                    type="text" 
                                    className="input input-sm input-bordered w-full" 
                                ...
                                <div className="text-sm opacity-80 leading-relaxed">{p.description || <span className="italic opacity-40">Keine Beschreibung</span>}</div>
                            )}
                        </div>

                        <div className="flex flex-col gap-1">
                            <span className="text-xs font-bold opacity-70">Wert</span>
                            {isBatchMode ? (
                ...
                {filteredProducts.length === 0 && (
                    <div className="text-center py-8 opacity-50 border border-dashed border-base-300 rounded-box text-sm">Keine Einträge gefunden.</div>
                )}
            </div>
            ...
            {isBatchMode && products.length > 0 && (
                <div className="mt-4 pt-4 border-t border-base-300 flex justify-end gap-2">
                    <button className="btn btn-sm btn-ghost" onClick={() => setIsBatchMode(false)} disabled={isSaving}>Abbrechen</button>
