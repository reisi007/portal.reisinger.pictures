...
export default function ManagementProductsView() {
    ...
    const handleSave = async (data: Partial<Product>) => {
        try {
            if (editingProduct) {
                ...
                showToast('success', 'Produkt aktualisiert');
            } else {
                await apiMutate('/api/management/products', 'POST', data);
                showToast('success', 'Produkt angelegt');
            }
            mutate();
        } catch (e: unknown) {
    ...
    const handleDelete = async (id: string) => {
        if (!(await confirm({ title: 'Produkt löschen?', message: 'Möchtest du dieses Produkt wirklich entfernen?', confirmColor: 'error' }))) return;
        try {
    ...
    if (error) return <div className="p-10"><ErrorMessage message="Fehler beim Laden der Produkte." /></div>;
    ...
    return (
        <div className="p-6 md:p-10 max-w-5xl mx-auto w-full">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    <h1 className="text-4xl font-bold mb-2">Produkte & Leistungen</h1>
                    <p className="opacity-70">Verwalte deine Standard-Leistungen für manuelle Rechnungen.</p>
                </div>
                <button className="btn btn-primary" onClick={() => { setEditingProduct(null); setIsModalOpen(true); }}>+ Neues Produkt</button>
            </div>
                    ...
                    <table className="table table-zebra w-full">
                        <thead>
                            <tr>
                                ...
                                <th className="text-right">Standard-Preis (Netto)</th>
                        ...
                        <tbody>
                            {filtered?.map(p => (
                                <tr key={p.id}>
                                    ...
                                    <td className="text-right font-mono font-bold text-primary">{p.price.toFixed(2)} €</td>
                            ...
                            {filtered?.length === 0 && (
                                <tr><td colSpan={3} className="text-center py-10 opacity-50">Keine Produkte gefunden.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
