...
import {EmailTemplate, useEmailTemplates} from '../../logic/useEmailTemplates';
    ...
    return (
        <div className="p-10 max-w-5xl mx-auto w-full flex flex-col gap-8">
            <div className="flex justify-between items-center">
                ...
                <button className="btn btn-primary" onClick={() => setEditing({name: '', subject: '', body: ''})}>+ Neue
                    Vorlage
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div className="card-body p-6">
                            ...
                            <div className="card-actions justify-end mt-4">
                                <button className="btn btn-ghost btn-xs text-error" onClick={() => {
                                    if (window.confirm('Löschen?')) deleteTemplate(tpl.id)
                                }}>Löschen
                                </button>
                                <button className="btn btn-secondary btn-xs"
                                        onClick={() => setEditing(tpl)}>Bearbeiten
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                {templates?.length === 0 &&
                    <div className="col-span-full text-center opacity-50">Noch keine Vorlagen angelegt.</div>}
            </div>
                        ...
                        <form onSubmit={handleSave} className="space-y-4">
                            ...
                            <div className="form-control">
                                <label className="label"><span
                                    className="label-text font-bold">E-Mail Betreff</span></label>
                                <input type="text" required value={editing.subject}
                            ...
                            <div className="form-control">
                                <label className="label">
                                    ...
                                    <span
                                        className="label-text-alt opacity-70">Variablen: {"{user_name}"}, {"{gallery_name}"}, {"{link}"}</span>
                                </label>
                                <textarea required value={editing.body}
                                          onChange={e => setEditing({...editing, body: e.target.value})}
                                          className="textarea textarea-bordered h-48 font-mono text-sm"></textarea>
                            </div>
                            <div className="modal-action">
                                <button type="button" className="btn btn-ghost"
                                        onClick={() => setEditing(null)}>Abbrechen
                                </button>
