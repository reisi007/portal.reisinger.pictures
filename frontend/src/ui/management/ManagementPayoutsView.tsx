...
export default function ManagementPayoutsView() {
    ...
    const handleCalculate = async (e: React.FormEvent) => {
        ...
        if (!(await confirm({
            title: 'Abrechnungslauf starten?',
            message: `Möchtest du den Pool für ${month}/${year} wirklich mit ${formatMoney(cents)} berechnen? Bestehende (nicht ausbezahlte) Statements für dieses Monat werden überschrieben.`,
            confirmText: 'Berechnen'
        }))) return;
    ...
    const statusBadge = (status: string) => {
        switch (status) {
            case 'pending':
                return <span className="badge badge-warning badge-sm">Wartet auf Freigabe</span>;
            case 'rollover':
                return <span className="badge badge-ghost badge-sm">&lt; 50€ (Rollover)</span>;
            case 'approved':
                return <span className="badge badge-info badge-sm">Freigegeben</span>;
            case 'paid':
                return <span className="badge badge-success badge-sm text-white">Ausbezahlt</span>;
            default:
                ...
                <form onSubmit={handleCalculate} className="flex flex-col md:flex-row items-end gap-4">
                    ...
                    <div className="form-control w-full md:w-48">
                        ...
                        <input type="number" step="0.01" value={netPool} onChange={e => setNetPool(e.target.value)}
                               placeholder="z.B. 2500.00" className="input input-bordered font-mono"
                               required/>
                    </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                <div className="xl:col-span-1">
                    ...
                    <div className="flex flex-col gap-4">
                        {data.pools.map(pool => (
                            <div key={pool.id} className="bg-base-100 p-4 rounded-box border border-base-300 shadow-sm">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-sm">
                                    <div className="opacity-70">Netto-Topf:</div>
                                    <div className="font-mono text-right">{formatMoney(pool.net_pool_cents)}</div>
                                    <div className="opacity-70">Unique DLs:</div>
                                    <div className="font-mono text-right">{pool.total_unique_downloads}</div>
                                    <div className="opacity-70">Shares gesamt:</div>
                                    <div className="font-mono text-right">{pool.total_shares}</div>
                                    <div className="opacity-70 font-bold border-t border-base-300 pt-1 mt-1">Wert pro
                                        Share:
                                    </div>
                                    <div
                        ...
                        {data.pools.length === 0 &&
                            <div className="opacity-50 text-sm">Noch keine Pools berechnet.</div>}
                    </div>
                </div>
                    ...
                    <div className="overflow-x-auto bg-base-100 rounded-box border border-base-300 shadow-sm">
                        <table className="table table-zebra w-full table-sm">
                                        ...
                                        <div className="flex flex-col gap-1 items-start">
                                            ...
                                            {stmt.status === 'pending' && (
                                                <button onClick={() => updateStatus(stmt.id, 'approve')}
                                                        className="btn btn-xs btn-outline mt-1">Freigeben</button>
                                            )}
                                            {stmt.status === 'approved' && (
                                                <button onClick={() => updateStatus(stmt.id, 'pay')}
                                                        className="btn btn-xs btn-success text-white mt-1">Als Bezahlt
                                                    markieren</button>
                                            )}
                                        </div>
                                    </td>
                            ...
                            {data.statements.length === 0 && <tr>
                                <td colSpan={5} className="text-center py-6 opacity-50">Keine Statements vorhanden.</td>
                            </tr>}
                            </tbody>
                        </table>
                    </div>
