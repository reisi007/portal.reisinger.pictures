...
export default function ManagementTenantDetailView() {
    ...
    const { tenant, isLoading, updateTenant, deleteTenant, syncUsers, syncGroups, generateCollectiveInvoice } = useTenants(id);
    ...
    const handleDelete = async () => {
        if (await confirm({ title: 'Mandant löschen?', message: 'Wirklich löschen? Zuweisungen gehen verloren (Nutzer und Ordner bleiben aber erhalten).', confirmColor: 'error' })) {
    ...
    const handleGenerateInvoice = async () => {
        if (await confirm({ title: 'Sammelrechnung erstellen?', message: 'Ausstehende Lieferscheine werden nun gebündelt, eine P-Rechnungsnummer erzeugt und als PDF an deine E-Mail versendet.', confirmText: 'Erstellen', confirmColor: 'primary' })) {
    ...
    return (
        ...
        <div className="p-6 md:p-10 max-w-6xl mx-auto w-full relative">
            <div className="flex items-center gap-4 mb-8">
                <button onClick={() => navigate('/tenants')} className="btn btn-circle btn-ghost shrink-0"><span className="iconify mdi--arrow-left text-2xl"></span></button>
                <div>
