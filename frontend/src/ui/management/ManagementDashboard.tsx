...
export default function ManagementDashboard() {
    ...
    const { tree, isLoading, isError, createGroup, createGallery, deleteGallery } = useProtectedGalleries();
                    ...
                    <ErrorBoundary>
                        ...
                        {currentView === 'structure' && (
                            <div className="p-6 md:p-10">
                                ...
                                <p className="opacity-70 text-lg mb-8">Erstelle eine Galerie oder eine Meta-Galerie. Wähle links eine Galerie aus der Struktur aus, um sie zu bearbeiten.</p>

                                {user?.is_photographer && (
                                    <div className="mt-12 border-t border-base-300 pt-8">
                                        <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
                                            <span className="iconify mdi--history text-primary"></span> Deine neuesten Uploads & Galerien
                                        </h2>
                                        {feedLoading ? (
                                                ...
                                                {personalFeed?.galleries && personalFeed.galleries.length > 0 && (
                                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                        {personalFeed.galleries.slice(0, 3).map(g => (
                                                            <Link key={g.id} to={'/' + g.full_path} className="card bg-base-100 shadow-sm hover:shadow-xl transition-all border border-base-300">
                                                ...
                                                {personalFeed?.photos && personalFeed.photos.length > 0 && (
                                                    <div className="grid grid-cols-3 md:grid-cols-6 lg:grid-cols-8 gap-2">
                                                        {personalFeed.photos.slice(0, 16).map(p => (
                                                            <Link key={p.id} to={'/photos/' + p.id} className="block relative aspect-square bg-base-300 rounded overflow-hidden group shadow-sm hover:shadow-md">
                                                                <img src={p.thumb_url} className="object-cover w-full h-full group-hover:scale-105 transition-transform" loading="lazy" />
                                                            </Link>
                                                        ))}
                                                    </div>
                                                )}
                                                {(!personalFeed?.galleries?.length && !personalFeed?.photos?.length) && (
                                                    <p className="opacity-50">Du hast noch keine eigenen Galerien oder Bilder.</p>
                                                )}
                                            </div>
                                        )}
                                    </div>
