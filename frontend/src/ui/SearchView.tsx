...
export default function SearchView() {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        if (localQuery.trim().length >= 2) setSearchParams({ q: localQuery.trim() });
    };
        ...
        <div className="flex h-screen bg-base-100 overflow-hidden relative">
            ...
            <main className="flex-1 overflow-y-auto flex flex-col w-full relative bg-base-200">
                <header className="p-4 border-b border-base-300 bg-base-100 flex items-center gap-4 sticky top-0 z-30 md:hidden">
                    ...
                    <h1 className="text-xl font-bold text-primary">Suche</h1>
                </header>
                ...
                <div className="container mx-auto max-w-7xl p-4 md:p-8">
                    ...
                    {/* Empty State, wenn kein Query existiert */}
                    {!query ? (
                        <div className="text-center py-20 opacity-60">
                            <span className="iconify mdi--folder-search-outline text-7xl mb-4 text-primary"></span>
                            <h2 className="text-2xl font-bold">Wonach suchst du?</h2>
                            <p className="mt-2">Gib einen Suchbegriff ein, um passende Galerien und Fotos zu finden.</p>
                        </div>
                    ) : (
                        <>
                            <h1 className="text-2xl md:text-3xl font-bold mb-8 text-center">
                                Suchergebnisse für <span className="text-primary">"{query}"</span>
                            </h1>

                            {isLoading && <div className="flex justify-center p-10"><span className="loading loading-spinner loading-lg text-primary"></span></div>}
                            {isError && <div className="alert alert-error">Fehler beim Laden der Suchergebnisse.</div>}
