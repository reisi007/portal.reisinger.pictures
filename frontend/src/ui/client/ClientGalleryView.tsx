...
export default function ClientGalleryView() {
    ...
    if (isError || !gallery) return <PageLayout>
        <div className="p-8 text-center text-error">Galerie nicht gefunden oder Zugriff verweigert.</div>
    </PageLayout>;

    return (
        <PageLayout>
            <div className="container mx-auto p-4 md:p-8 relative">
                <button onClick={() => navigate('/')} className="btn btn-sm btn-ghost mb-4">&larr; Zurück zur
                    Übersicht
                </button>
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                    ...
                    <div className="flex gap-2">
                        {gallery.type === 'selection' && user && (
                            <button onClick={handleFinishRating} disabled={finishing}
                                    className="btn btn-success text-white">
                                {finishing ? <span className="loading loading-spinner"></span> :
                                    <span className="iconify mdi--check-all"></span>}
                                Auswahl abschließen
                            </button>
                        )}
                        {gallery.type === 'delivery' && totalPhotos > 0 && (
                            <button
                                onClick={() => window.open('/api/galleries/' + gallery.id + '/download-zip', '_self')}
                ...
                {!isLoading && photos.length === 0 && (
                    <div
                        className="py-20 text-center flex flex-col items-center justify-center opacity-70 bg-base-200 rounded-box border border-base-300 mb-8">
                        ...
                        {gallery.is_live ? (
                            <p className="mt-2 text-warning flex items-center gap-2">
                                ...
                                Dies ist eine LIVE Galerie. Sobald der Fotograf Bilder hochlädt, erscheinen sie hier
                                (automatischer Refresh alle 10s).
                            </p>
                        ) : (
                                    ...
                                    user ? (
                                        <>
                                            <div className="rating rating-sm justify-center">
                                                {[1, 2, 3, 4, 5].map(star => (
                                                    <input key={star} type="radio"
                                                           className={"mask mask-star-2 " + (photo.rating >= star ? 'bg-orange-400' : 'bg-base-300')}
                                                           checked={photo.rating === star}
                                                           onChange={() => ratePhoto(photo.id, star, photo.comment || '')}/>
                                                ))}
                                            </div>
                                            <input type="text" placeholder="Kommentar..."
                                                   defaultValue={photo.comment || ''}
                                                   onBlur={(e) => ratePhoto(photo.id, photo.rating || 0, e.target.value)}
                                                   className="input input-bordered input-sm w-full"/>
                                        </>
                                    ) : null
                                ) : (
                                    <button
                                        onClick={() => window.open('/api/photos/' + photo.id + '/download', '_self')}
                                        className="btn btn-secondary btn-sm">Einzel-Download</button>
                                )}
                            </div>
                        </div>
                    ))}
