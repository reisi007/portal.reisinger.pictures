            ...
            <div className="modal-box relative max-w-2xl">
                ...
                <div className="flex flex-col gap-6 mb-6 bg-base-200 p-5 rounded-box border border-base-300">
                    ...
                    <div className="form-control w-full">
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <div className="dropdown dropdown-hover dropdown-end">
                                <label tabIndex={0} className="btn btn-circle btn-ghost btn-xs text-info"><span className="mdi--information-outline text-lg"></span></label>
                                <div tabIndex={0} className="dropdown-content z-[20] card card-compact w-72 p-2 shadow-xl bg-base-100 border border-base-300 text-sm">
                        ...
                        <div className="join w-full shadow-sm">
                            <button className={`btn join-item btn-sm flex-1 ${usage === 'editorial' ? 'btn-primary' : 'btn-outline bg-base-100'}`} onClick={() => setUsage('editorial')}>Redaktionell (x1)</button>
                            <button className={`btn join-item btn-sm flex-1 ${usage === 'commercial' ? 'btn-primary' : 'btn-outline bg-base-100'}`} onClick={() => setUsage('commercial')}>Kommerziell (x3)</button>
                        </div>
                    </div>
                    ...
                    <div className="form-control w-full">
                        ...
                        <div className="join w-full shadow-sm">
                            <button className={`btn join-item btn-sm flex-1 ${duration === '1_year' ? 'btn-primary' : 'btn-outline bg-base-100'}`} onClick={() => setDuration('1_year')}>1 Jahr (x1)</button>
                            <button className={`btn join-item btn-sm flex-1 ${duration === 'unlimited' ? 'btn-primary' : 'btn-outline bg-base-100'}`} onClick={() => setDuration('unlimited')}>Unbegrenzt (x2)</button>
                        </div>
                    </div>
                            ...
                            <div key={tier.id} className={`p-4 rounded-box border flex flex-col md:flex-row items-start md:items-center justify-between gap-4 transition-colors ${covered ? 'border-success bg-success/5' : 'border-base-300 bg-base-100'}`}>
                                <div>
                                    <h4 className="font-bold text-lg flex items-center gap-2">
                                        {tier.label}
                                        {covered && <span className="badge badge-success badge-sm text-white">Inklusive</span>}
                                    </h4>
                                    <p className="text-sm opacity-70">{tier.desc}</p>
                                </div>
                                <div className="shrink-0 w-full md:w-auto text-right">
                                    {covered ? (
                                        <button onClick={() => handleDownload(tier.id)} className="btn btn-success w-full text-white">
                                            <span className="iconify mdi--download"></span> Sofort Download
                                        </button>
                                    ) : canBuy ? (
                                        <button onClick={() => handleAddToCart(tier.id, upgradePrice)} className="btn btn-primary w-full">
                                            <span className="iconify mdi--cart-plus"></span> + {upgradePrice.toFixed(2)} €
                                        </button>
                                    ) : (
                                        <button disabled className="btn btn-disabled w-full" title="Deine Berechtigung erlaubt keine Upgrade-Käufe.">
