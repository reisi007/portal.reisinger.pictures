...
test.describe('SRP Brand Dashboard (G12)', () => {
    ...
    test('SRP admin sidebar has no B2B admin items', { tag: ['@feature:brand:dashboard'] }, async ({ page }) => {
    ...
    test('SRP admin is redirected from B2B routes', { tag: ['@feature:brand:dashboard'] }, async ({ page }) => {
        test.setTimeout(30000);
        const auth = new AuthHelper(page);
        await auth.login(adminUser.email, adminUser.password, 'http://buy.localhost:4321/');
    ...
    test('SRP client sidebar has no admin items', { tag: ['@feature:brand:dashboard'] }, async ({ page }) => {
        test.setTimeout(30000);
