...
test.describe('AI Batch Edit Modal (Server-Side)', () => {
    ...
    test('shows AI Batch-Edit button for photographer in delivery gallery', { tag: ['@feature:photographer:ai'] }, async ({page}) => {
    ...
    test('opens AI Batch-Edit modal when button is clicked', { tag: ['@feature:photographer:ai'] }, async ({page}) => {
        const auth = new AuthHelper(page);
        await auth.login(testUser.email, testUser.password);
