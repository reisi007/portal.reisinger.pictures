...
test.describe('SRP Volume Pricing', () => {
    ...
    test('Frontend: VolumeLicensingCard auf SRP-Brand', async ({ page }) => {
    ...
    test('Pricing-Verifikation: Staffel 1/2/3 mit retroaktivem Volumenpreis', async ({ page }) => {
    ...
    test('API: Login mit SRP-Brand bestätigt brand=srp', async ({ request }) => {
    ...
    test('API: RP-Checkout unverändert (rp-Smoke Regression)', async ({ request }) => {
    ...
    test('Stripe-Integration: SRP-Volumenbetrag in Stripe-Session (12 Fotos × Tier 2)', async ({ page, request }) => {
        test.setTimeout(120000);
        const auth = new AuthHelper(page);
