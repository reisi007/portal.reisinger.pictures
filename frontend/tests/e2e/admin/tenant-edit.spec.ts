...
test.describe('E2: Org-Admin editiert Org Settings', () => {
    ...
    test('Org-Admin editiert Org Name und speichert', async ({ page, request }) => {
        ...
        const orgCard = page.locator('main .card').filter({ hasText: tenantName }).first();
        await expect(orgCard).toBeVisible({ timeout: 10000 });
        await orgCard.click();

        await expect(page.locator('h1:has-text("' + tenantName + '")')).toBeVisible({ timeout: 10000 });

        const newName = `Updated ${Math.random().toString(36).substring(2, 10)}`;
        const nameInput = page.locator('input[value="' + tenantName + '"]');
